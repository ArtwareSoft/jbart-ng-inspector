(function() {
	...
	window.jbart = window.jbart || { comps: {} };
	jbart.comps = jbart.comps || {};
	...
	document.addEventListener("DOMContentLoaded",function() {
		jbn_initJQuery(function() {
			...
			setTimeout(jbn_openStudioMainWindow,100);
		});
	},false);

	function jbn_initJQuery(callback) {
		if (window.$) return callback();
		var el = document.createElement('script'); el.src = '//ajax.googleapis.com/ajax/libs/jquery/1.8.3/jquery.min.js'; document.head.appendChild(el);
...
jb_component('dialogFeature.maxZIndexOnClick', {
	...
	impl: function(context) {
		...
		jb_bind(dialog, 'attach', function() {
			...
			dialog.$el.click(setAsMaxZIndex);
		});
...
function jb_doOnCodeMirrorObject(func) {
	if (window.CodeMirror) return func(window.CodeMirror);
...
jb_component('studio.angularProperties', {
	...
	impl: function(context,templateElem,_scope) {
		...
		var html = jb_getHtmlImport('studio/html/properties.html');
		var out = window.ngService.$compile(html)(scope);
		scope.$digest();
		return jb_simpleControl(context,$(out));
	}
...
function jbn_studioHtmlElem(htmlName,controllerOrProps,parent) {
	...
	if (parent) parent.append(compiled);
	scope.$digest();
	return compiled;
}
...
function jbn_openStudioMainWindow() {
	jb_openStudioPopupWindow({
		...
		content: function(parent) { 
			$(parent).append(jbn_studioHtmlElem('main-window',addToScope,parent));
		}
	})

	function addToScope($scope) {
		$scope.inspectData = jbn_inspectData();
		$scope.showTemplates = jbn_showTemplates;
		$scope.select = jbn_select;
	}
}

function jbn_showTemplates() {
	alert('show templates');
}
	...
	impl: {
		...
		content: {
			...
			features: [
			  ...
			  { $treeDoubleClickNode: { $: 'studio-ng.openTemplatePopup', templateElem: '{{$treeElem}}' } },
			  { $id: 'studio-tree' }
			]
		}
	}
